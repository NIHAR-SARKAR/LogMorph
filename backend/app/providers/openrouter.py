"""OpenRouter provider adapter using raw httpx."""
...
class OpenRouterAdapter(BaseLLMClient):
    ...
    def _validate_config(self, config: ResolvedLLMConfig) -> None:
        ...
        if config.base_url:
            validate_provider_url(config.base_url, "OpenRouter", "https://openrouter.ai/api")

    def _get_client(self, config: ResolvedLLMConfig) -> httpx.AsyncClient:
        self._validate_config(config)
        key = decrypt_api_key(config.api_key_encrypted)
        base = (config.base_url or "https://openrouter.ai/api").rstrip("/")
        extra = config.extra_headers or {}
        referer = extra.get("http_referer", "https://logmorph.ai") if isinstance(extra, dict) else "https://logmorph.ai"
        title = extra.get("site_title", "LogMorph AI") if isinstance(extra, dict) else "LogMorph AI"
        cache_key = f"openrouter:{base}:{key[:8]}"
    ...
    async def generate(
        ...
        body = {
            "model": config.model_id or "openai/gpt-4o",
            "messages": messages,
    ...
    async def test_connection(self, config: ResolvedLLMConfig) -> Dict[str, Any]:
        try:
            ...
            logger.error("OpenRouter test connection error: %s", e)
