"""OpenAI provider adapter using raw httpx (not the openai SDK)."""
...
class OpenAIAdapter(BaseLLMClient):
    ...
    def _validate_config(self, config: ResolvedLLMConfig) -> None:
        ...
        if config.base_url:
            validate_provider_url(config.base_url, "OpenAI", "https://api.openai.com")
    ...
    async def test_connection(self, config: ResolvedLLMConfig) -> Dict[str, Any]:
        try:
            ...
            logger.error("OpenAI test connection error: %s", e)
