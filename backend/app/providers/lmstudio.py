"""LM Studio provider adapter using raw httpx."""
...
class LMStudioAdapter(BaseLLMClient):
    name = "lmstudio"
    ...
    async def generate(
        ...
        body = {
            "model": config.model_id or "local-model",
            "messages": messages,
    ...
    async def test_connection(self, config: ResolvedLLMConfig) -> Dict[str, Any]:
        try:
            ...
            logger.error("LM Studio test connection error: %s", e)
